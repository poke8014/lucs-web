...
9. **Nothing machine-readable about location ever persists (bed planner 2026-07-07; satellite underlay 2026-07-11).** The planner's address path and sun simulation never store an address, lat/lng, parcel id, or tile coords — nothing machine-readable about location ever persists. Raster imagery the user explicitly chose to keep (path-2 uploads, kept satellite photos) is allowed, geo-metadata stripped — pixels and an attribution string only. Saved `GardenPlan`s otherwise carry de-identified feet-space geometry; solar math stays on the fixed regional latitude constant (South Bay v1). Don't "improve" sun accuracy by persisting user coordinates. The privacy contract is a hard requirement; full detail in [planning/sunshower_bed_planner_spec.md](../planning/sunshower_bed_planner_spec.md) and [planning/sunshower_satellite_underlay_spec.md](../planning/sunshower_satellite_underlay_spec.md).
