...
3. **Plant-page frontmatter is the canonical schema.** `src/data/plants.json` is *derived* (`node ops/build-plant-data.mjs`) and the future Supabase tables will be derived the same way. Never hand-edit `plants.json`; regenerate it whenever plant frontmatter changes. **Regeneration is a chain, not one command** (discovered 2026-07-06): the cleanup-plan removal fields are a post-build overlay applied by four Python scripts run *after* the build script — the exact chain is documented in the build-script header. A bare build-script run silently wipes them.
...
- **`plants.json` regen wipes the removal overlay if you stop at the build script.** The 61 records with cleanup-plan removal fields get them from four post-build Python appliers (chain in the `ops/build-plant-data.mjs` header). Found during the 2026-07-06 selector build — the regen there verified byte-identical removal fields after the full chain.
