---
type: plant
title: Eriogonum gracile (Slender Woolly Buckwheat)
aliases: []
tags: [native, annual, pollinator-plant]
status: stub
sources:
  - https://calscape.org/export/search/?lat=<lat>&lng=<lng>  # Calscape export — see sources/calscape.md
last_updated: 2026-05-17

scientific_name: Eriogonum gracile
common_names: [Slender Woolly Buckwheat]
plant_type: annual
nativity: native
pollinators: [bees, butterflies]
water: low
sun: full
soil: [well-drained]
bloom_season: [spring, summer, fall]
height_ft: "1 - 2"
width_ft: "0.3 - 0.7"
regions: []
host_plant_for: []  # named lep species — pending Calscape /host enrichment scrape

# PROPOSED schema block (parallel to invasive:) — confirm in sources/calscape.md
native:
  butterflies_moths_supported: 47
  attracts_wildlife: [Bees, Birds, Butterflies, Caterpillars]
  plant_type_raw: "Annual herb"
  sun_range: "Full Sun"
  water_range: "Low, Very Low"
  ease_of_care: ""
  soil_drainage: [Fast]
  soil_ph: "5.2 - 8.2"
  communities: [Chaparral, Coastal Sage Scrub, Foothill Woodland, Southern Oak Woodland]
  communities_simplified: [Chaparral, Coastal Scrub, Woodland]
  companions: []
  sunset_zones: ""
  hardiness: ""
  nursery_availability: "Sometimes Available"
  rarity: ""
  is_cultivar: false
  jepson_url: "https://ucjeps.berkeley.edu/eflora/eflora_display.php?name=Eriogonum+gracile"
  calscape_url: "https://calscape.org/Eriogonum-gracile-%28Slender-Woolly-Buckwheat%29"
  retrieved: 2026-05-17
---

# Eriogonum gracile (Slender Woolly Buckwheat)

Eriogonum gracile (Slender Woolly Buckwheat) is a California-native annual of Chaparral, Coastal Scrub, Woodland. Calscape records **47** butterfly/moth species supported. _Structured data only — full description and named host species pending the Calscape page-scrape enrichment pass (see [[sources/calscape]])._

## Identify

- **Height:** 1 - 2 ft
- **Width:** 4 - 8 in
- **Flower color:** Cream, Pink, White, Yellow, Brown
- **Flowering season:** Spring, Summer, Fall
- **Seasonality:** Winter Semi-deciduous

## Grow

- **Sun:** Full Sun
- **Water:** Low, Very Low
- **Soil drainage:** Fast
- **Soil pH:** 5.2 - 8.2
- **Tips:** Prefers sun, part shade. Likes sandy soils.

## Wildlife value

- **Pollinators:** bees, butterflies
- **Butterflies & moths supported:** 47 (named species pending enrichment)
- **Attracts wildlife:** Bees, Birds, Butterflies, Caterpillars

## Where it belongs

- **Plant communities:** Chaparral, Coastal Sage Scrub, Foothill Woodland, Southern Oak Woodland
- **Site type:** Dry, sandy places, variable
- **Nursery availability:** Sometimes Available

## Photos

### iNaturalist

![](../raw/assets/inaturalist/eriogonum-gracile/eriogonum-gracile_01_inat-589536313.jpg)
*(c) klicklo, some rights reserved (CC BY) — [observation](https://www.inaturalist.org/observations/325595622)*

![](../raw/assets/inaturalist/eriogonum-gracile/eriogonum-gracile_02_inat-424674758.jpeg)
*no rights reserved — [observation](https://www.inaturalist.org/observations/238530168)*

![](../raw/assets/inaturalist/eriogonum-gracile/eriogonum-gracile_03_inat-563824101.jpg)
*no rights reserved — [observation](https://www.inaturalist.org/observations/312294985)*

![](../raw/assets/inaturalist/eriogonum-gracile/eriogonum-gracile_04_inat-8658875.jpg)
*(c) Adam J. Searcy, some rights reserved (CC BY) — [observation](https://www.inaturalist.org/observations/6805336)*

![](../raw/assets/inaturalist/eriogonum-gracile/eriogonum-gracile_05_inat-568485343.jpg)
*(c) jayjt, some rights reserved (CC BY) — [observation](https://www.inaturalist.org/observations/314733765)*

## Sources

- Calscape (CNPS) — [Eriogonum gracile](https://calscape.org/Eriogonum-gracile-%28Slender-Woolly-Buckwheat%29). Structured fields from the
  location-filtered Calscape export; see [[sources/calscape]] for the dataset
  meta-page, retrieval recipe, and licensing.
- Jepson eFlora — [taxon record](https://ucjeps.berkeley.edu/eflora/eflora_display.php?name=Eriogonum+gracile).
