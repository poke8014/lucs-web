---
type: plant
title: Trifolium barbigerum (Bearded Clover)
aliases: []
tags: [native, annual, pollinator-plant]
status: stub
sources:
  - https://calscape.org/export/search/?lat=<lat>&lng=<lng>  # Calscape export — see sources/calscape.md
last_updated: 2026-05-17

scientific_name: Trifolium barbigerum
common_names: [Bearded Clover]
plant_type: annual
nativity: native
pollinators: [butterflies]
water:
sun:
soil: []
bloom_season: [spring, summer]
height_ft: ""
width_ft: ""
regions: []
host_plant_for: []  # named lep species — pending Calscape /host enrichment scrape

# PROPOSED schema block (parallel to invasive:) — confirm in sources/calscape.md
native:
  butterflies_moths_supported: 57
  attracts_wildlife: [Butterflies, Caterpillars]
  plant_type_raw: "Annual herb"
  sun_range: ""
  water_range: ""
  ease_of_care: ""
  soil_drainage: []
  soil_ph: ""
  communities: [Closed-cone Pine Forest, Coastal Prairie, Mixed Evergreen Forest, Wetland-Riparian]
  communities_simplified: [Forest, Grassland, Wetland/Riparian]
  companions: []
  sunset_zones: ""
  hardiness: ""
  nursery_availability: "Never or Almost Never Available"
  rarity: ""
  is_cultivar: false
  jepson_url: "https://ucjeps.berkeley.edu/eflora/eflora_display.php?name=Trifolium+barbigerum"
  calscape_url: "https://calscape.org/Trifolium-barbigerum-%28Bearded-Clover%29"
  retrieved: 2026-05-17
---

# Trifolium barbigerum (Bearded Clover)

Trifolium barbigerum (Bearded Clover) is a California-native annual of Forest, Grassland, Wetland/Riparian. Calscape records **57** butterfly/moth species supported. _Structured data only — full description and named host species pending the Calscape page-scrape enrichment pass (see [[sources/calscape]])._

## Identify

- **Flower color:** Purple, Pink, White
- **Flowering season:** Summer, Spring
- **Seasonality:** Evergreen

## Wildlife value

- **Pollinators:** butterflies
- **Butterflies & moths supported:** 57 (named species pending enrichment)
- **Attracts wildlife:** Butterflies, Caterpillars

## Where it belongs

- **Plant communities:** Closed-cone Pine Forest, Coastal Prairie, Mixed Evergreen Forest, Wetland-Riparian
- **Nursery availability:** Never or Almost Never Available

## Photos

### iNaturalist

![](../raw/assets/inaturalist/trifolium-barbigerum/trifolium-barbigerum_01_inat-443653424.jpeg)
*(c) Cayman Lanzone, some rights reserved (CC BY) — [observation](https://www.inaturalist.org/observations/248313306)*

![](../raw/assets/inaturalist/trifolium-barbigerum/trifolium-barbigerum_02_inat-278595393.jpg)
*no rights reserved — [observation](https://www.inaturalist.org/observations/161186952)*

![](../raw/assets/inaturalist/trifolium-barbigerum/trifolium-barbigerum_03_inat-443655034.jpeg)
*(c) Cayman Lanzone, some rights reserved (CC BY) — [observation](https://www.inaturalist.org/observations/248313306)*

![](../raw/assets/inaturalist/trifolium-barbigerum/trifolium-barbigerum_04_inat-443655084.jpeg)
*(c) Cayman Lanzone, some rights reserved (CC BY) — [observation](https://www.inaturalist.org/observations/248313306)*

![](../raw/assets/inaturalist/trifolium-barbigerum/trifolium-barbigerum_05_inat-443655121.jpeg)
*(c) Cayman Lanzone, some rights reserved (CC BY) — [observation](https://www.inaturalist.org/observations/248313306)*

## Sources

- Calscape (CNPS) — [Trifolium barbigerum](https://calscape.org/Trifolium-barbigerum-%28Bearded-Clover%29). Structured fields from the
  location-filtered Calscape export; see [[sources/calscape]] for the dataset
  meta-page, retrieval recipe, and licensing.
- Jepson eFlora — [taxon record](https://ucjeps.berkeley.edu/eflora/eflora_display.php?name=Trifolium+barbigerum).
