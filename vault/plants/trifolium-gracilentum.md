---
type: plant
title: Trifolium gracilentum (Pinpoint Clover)
aliases: [Slender Clover]
tags: [native, annual, pollinator-plant]
status: stub
sources:
  - https://calscape.org/export/search/?lat=<lat>&lng=<lng>  # Calscape export — see sources/calscape.md
last_updated: 2026-05-17

scientific_name: Trifolium gracilentum
common_names: [Pinpoint Clover]
plant_type: annual
nativity: native
pollinators: [bees, butterflies]
water: moderate
sun: full
soil: []
bloom_season: [spring]
height_ft: ""
width_ft: ""
regions: []
host_plant_for: []  # named lep species — pending Calscape /host enrichment scrape

# PROPOSED schema block (parallel to invasive:) — confirm in sources/calscape.md
native:
  butterflies_moths_supported: 64
  attracts_wildlife: [Bees, Butterflies, Caterpillars]
  plant_type_raw: "Annual herb"
  sun_range: "Full Sun"
  water_range: "Moderate, High"
  ease_of_care: ""
  soil_drainage: []
  soil_ph: ""
  communities: [Chaparral, Foothill Woodland, Valley Grassland]
  communities_simplified: [Chaparral, Grassland, Woodland]
  companions: []
  sunset_zones: ""
  hardiness: ""
  nursery_availability: "Sometimes Available"
  rarity: ""
  is_cultivar: false
  jepson_url: "https://ucjeps.berkeley.edu/eflora/eflora_display.php?name=Trifolium+gracilentum"
  calscape_url: "https://calscape.org/Trifolium-gracilentum-%28Pinpoint-Clover%29"
  retrieved: 2026-05-17
---

# Trifolium gracilentum (Pinpoint Clover)

Trifolium gracilentum (Pinpoint Clover) is a California-native annual of Chaparral, Grassland, Woodland. Calscape records **64** butterfly/moth species supported. _Structured data only — full description and named host species pending the Calscape page-scrape enrichment pass (see [[sources/calscape]])._

## Identify

- **Flower color:** Purple, Pink
- **Flowering season:** Spring

## Grow

- **Sun:** Full Sun
- **Water:** Moderate, High
- **Tips:** Requires moist soil. Prefers sun.

## Wildlife value

- **Pollinators:** bees, butterflies
- **Butterflies & moths supported:** 64 (named species pending enrichment)
- **Attracts wildlife:** Bees, Butterflies, Caterpillars

## Where it belongs

- **Plant communities:** Chaparral, Foothill Woodland, Valley Grassland
- **Site type:** Moist grassy places and other wet places
- **Nursery availability:** Sometimes Available

## Photos

### iNaturalist

![](../raw/assets/inaturalist/trifolium-gracilentum/trifolium-gracilentum_01_inat-190776341.jpg)
*(c) Trevor Van Loon, some rights reserved (CC BY) — [observation](https://www.inaturalist.org/observations/112926557)*

![](../raw/assets/inaturalist/trifolium-gracilentum/trifolium-gracilentum_02_inat-211615571.jpeg)
*(c) Konshau Duman, some rights reserved (CC BY) — [observation](https://www.inaturalist.org/observations/124825427)*

![](../raw/assets/inaturalist/trifolium-gracilentum/trifolium-gracilentum_03_inat-36797415.jpeg)
*(c) Millie Basden, some rights reserved (CC BY) — [observation](https://www.inaturalist.org/observations/23785791)*

![](../raw/assets/inaturalist/trifolium-gracilentum/trifolium-gracilentum_04_inat-190776874.jpg)
*(c) Trevor Van Loon, some rights reserved (CC BY) — [observation](https://www.inaturalist.org/observations/112926557)*

![](../raw/assets/inaturalist/trifolium-gracilentum/trifolium-gracilentum_05_inat-190776895.jpg)
*(c) Trevor Van Loon, some rights reserved (CC BY) — [observation](https://www.inaturalist.org/observations/112926557)*

## Sources

- Calscape (CNPS) — [Trifolium gracilentum](https://calscape.org/Trifolium-gracilentum-%28Pinpoint-Clover%29). Structured fields from the
  location-filtered Calscape export; see [[sources/calscape]] for the dataset
  meta-page, retrieval recipe, and licensing.
- Jepson eFlora — [taxon record](https://ucjeps.berkeley.edu/eflora/eflora_display.php?name=Trifolium+gracilentum).
