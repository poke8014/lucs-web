...
**Last updated:** 2026-05-15 — iNat photo pull for the 21 new UC IPM residential weeds landed on `feat/inat-photos-ucipm-weeds`. 105 new photos; no Wikimedia fallback needed. New `vault/scripts/sync_inat_photos_to_plants_json.py` brings iNat metadata into `plants.json` `photos[]` and also resyncs the 137 Cal-IPC plants (3 → 5 photos each from the 2026-05-09 top-up that hadn't been reflected). 158/158 picker rows now at 5 photos. Prior entry: `plants.json` wiring for the 22 weeds (PR #11, merged) — 21 new picker rows + Layer A/B/C/sources, `prune_host_below_attachment` enum for dodder.
