...
**Current state (2026-07-05):** **Phase-3 bed-planner build spec landed** ([sunshower_bed_planner_spec.md](sunshower_bed_planner_spec.md), branch `docs/phase3-bed-planner-spec`) — the planner that consumes the site-inventory output: base map → paths-first sections → per-section density/phase-order → layered placement → checker → grown-in 3D view; work units A–I, two sign-offs pending (see the App/UI item). It restates the direction from the **still-unmerged `docs/phase-specs` branch** (M0–M7 milestone spine + 4 concept pages, in a second worktree) — that branch predates the walkthrough merge and needs a rebase-or-cherry-pick decision from Luc. The day before (2026-07-04), the **site-inventory walkthrough shipped** (`feat/site-inventory-mvp`) — `/sunshower/site-inventory` is a full 8-step guided wizard (Rainer & West archetype key → measured inventory → site-profile summary) producing a versioned localStorage `SiteProfile`, the input contract the Phase-2 selector and the bed planner consume; build record in [sunshower_site_inventory_mvp.md](sunshower_site_inventory_mvp.md). Vitest landed as the repo's first test runner (`npm test`, wired into CI). Earlier the same week, the *Post-Wild World* design-theory ingest merged (PR #24: 7 new concept pages + `sociability:` schema field). Phase 1 (Cleanup) closed 2026-05-18; Phase-2 data layer is live (150 Calscape native stubs, PR #19). Next up under *Native plant ingest*: Calscape enrichment scrape (in flight on `feat/calscape-enrichment`) → Phase-2 selector design. Prior milestones: see the archive changelog.
