...
**Last updated:** 2026-05-15 — `plants.json` wiring for the 22 UC IPM residential weeds landed on `feat/sync-residential-weeds-plants-json` (this PR). 21 new picker rows + Layer A/B/C/sources for all 22 (incl. dyer's woad upgrade). New `RemovalMethod` value `prune_host_below_attachment` introduced for parasitic dodder. Picker DB grew 137 → 158 plants; annotated subset 38 → 60.
...
- 📋 **iNat photo pull for new weeds** — same recipe as the Cal-IPC 137-plant pull. Wikimedia Commons fallback likely needed for some cosmopolitan weeds (dandelion, chickweed, plantain). Unblocked now that the picker rows exist.
