...
- 📋 **In-app site-inventory walkthrough — spec ready, dispatchable.** Expose [vault/concepts/site-inventory.md](../vault/concepts/site-inventory.md) as a guided in-app flow — aspect (compass-app prompt), sun map (hourly-photo prompt → 5-tier zoning), wind, slope, utilities, sightlines, soil/pH indicator-plant prompt (*"see any rhodos or lavenders nearby?"*), now opened by the Rainer & West archetype key ([vault/concepts/landscape-archetypes.md](../vault/concepts/landscape-archetypes.md)). Reusable from any phase route; sits between cleanup and selection. **Full build plan in [sunshower_site_inventory_mvp.md](sunshower_site_inventory_mvp.md)** (2026-07-04) — scope locked w/ Luc: multi-step wizard, localStorage `SiteProfile` contract, no in-app visuals (sketch/canvas deferred), 4 agent work packages in 2 rounds. Idea source: [vault/sources/gardenersworld-soil-ph.md](../vault/sources/gardenersworld-soil-ph.md) + [vault/sources/summerwinds-sun-exposure.md](../vault/sources/summerwinds-sun-exposure.md).
