...
- 📋 **WRIC wiki ingest** — Cal-IPC PAFs explicitly omit management info; WRIC has the short-term/long-term removal guidance. Populates the Identify / Remove / Prevent sections of plant pages and lands a `sources/wric.md` dataset meta-page. Blocked on PDF reader: WRIC PDFs are AES-encrypted (open-without-password, viewable but not directly text-extractable), so the ingest pass needs a PDF-aware extraction step.
...
- 📋 Phase-1 management concepts (removal-techniques, reinfestation-prevention, fire-risk, toxicity-handling, disposal) — blocked on WRIC ingest landing meaningful content.
