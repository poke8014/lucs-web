...
**Last updated:** 2026-05-16 — GUI-MVP doc closed out: its remaining open tasks were migrated into this backlog — Agent D §2–3 (prep-for-planting + coming-next placeholder) under *Cleanup-plan rendering → sections 2–3*; the post-DB *Deferred* set as a *Plan enrichment — post-DB* icebox entry under *App / UI*. [sunshower_gui_mvp.md](sunshower_gui_mvp.md) is now a closed per-agent build record (no new tasks there). Same day: Agent C shipped on `feat/sunshower-yard-state-router` (PR #16, merged): `/sunshower` now leads with a "How's your yard looking today?" dropdown that routes overgrown/partial → cleanup-plan (`?yard=`), bare → new `/sunshower/site-inventory` interstitial, established → new `/sunshower/care` interstitial; shipped as a dropdown (not the spec'd free-text matcher) + dedicated route pages (not inline wiki links). Plus a full responsive/mobile pass — desktop keeps the fixed immersive 3D scene, mobile is a scrollable doc with an in-flow shovel stage and a continuous ambient `backdrop` Scene variant. Unblocks Agent D's "prep for planting" section. Prior entry: Doc-lint pass on `chore/lint-stale-plant-counts`: synced stale "137 plants" claims in [PlantPicker.tsx](../src/app/sunshower/cleanup-plan/PlantPicker.tsx), [page.tsx](../src/app/sunshower/cleanup-plan/page.tsx), [plan.ts](../src/app/sunshower/cleanup-plan/plan.ts), and [planning/sunshower/resources.md](sunshower/resources.md) to the current 158-plant catalog. New App/UI item added: picker-scope de-emphasis + advanced-search toggle for non-residential invasives. Prior entry: stepper-nav fix on `fix/cleanup-plan-stepper-nav` (PR #14, merged) — sync picks on back-nav, gated forward-nav. Earlier same day: Beginner-gardening wiki ingest on `feat/beginner-gardening-ingest` (PR #13): 4 sources, 5 new concepts (garden-zoning, wildlife-coexistence, planting-technique + composting/pruning stubs), 4 concept updates, planning-doc ideas section. New App/UI backlog items added for homepage yard-state router, in-app site-inventory walkthrough, parallel-tracks UX, friendly-to-natives copy pass, Phase 4 (Care) gap. Earlier: iNat photo pull for the 21 new UC IPM residential weeds (`feat/inat-photos-ucipm-weeds`); 158/158 picker rows at 5 photos. `plants.json` wiring for the 22 weeds (PR #11, merged) — 21 new picker rows + Layer A/B/C/sources, `prune_host_below_attachment` enum for dodder.
...
- 🚧 **User-zero MVP — pick list → confirmed list → attack plan.** Phase 1 entry point at [/sunshower/cleanup-plan](../src/app/sunshower/cleanup-plan/page.tsx). Three steps:
  ...
  3. 🚧 **Plan.** Action plan grouped by `removal_method` (Layer A) + yard-wide summary card grid for timing / tools / cautions / multi-year follow-up (Layer B). Still pending: yard-state-driven "prep the area for planting" section (Agent C shipped PR #16 — now unblocked; yard state arrives via `?yard=`) and a "coming next" placeholder (Phase 2 handoff). Tracked in detail under *Cleanup-plan rendering → sections 2–3*.
...
- 📋 **Sections 2–3 — "Prep the area for planting" + "Coming next" (migrated from GUI-MVP Agent D §2–3, 2026-05-16; unblocked by Agent C PR #16).** Layers A/B/C deliver Section 1 (grouped removal) + the yard-wide summary. Remaining: **(2) Prep the area for planting** — generic, yard-state-driven guidance: cleanup-method recommendation, "don't compost rhizome fragments", "leave existing trees alone", what "ready to plant" looks like. Yard state now reaches cleanup-plan via the `?yard=` query param (`overgrown` | `partial`). **(3) Coming next** — an explicit placeholder section for the deferred timing / soil-prep / plant-selection items (Phase 2 handoff; the data feed is tracked under *Plan enrichment — post-DB* in App / UI). **Anti-goal (carried from the GUI-MVP spec):** no per-plant detail card / no "plant + photo + method" repeating block — the plan step stays action-first; photos live on the confirm step. Design rationale: [sunshower_gui_mvp.md → Agent D](sunshower_gui_mvp.md).
