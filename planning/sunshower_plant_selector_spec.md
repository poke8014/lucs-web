...
|---|---|---|
| **A** ✅ 2026-07-06 | **Data & logic core.** plants.json regeneration + build-script extension (shared list above; flag the picker change); `sun_range` string enumeration + tier↔range matrix; archetype↔community mapping; `fitForZone` / `similarNatives` / `contribution` as pure functions — all vitest-covered against the real 150-plant corpus, not fixtures | — |
| **B** ✅ 2026-07-06 | **Selector surface.** Route + server wrapper, lens header (zone picker, community chips), role batches with reroll + facet toggles, search, detail drawer + `?plant=` deep link, mobile-first pass | A |
| **C** ✅ 2026-07-06 | **Nudge & contribution UX.** Tier ladder behaviors end-to-end, contribution card (incl. 🟡/🔴/unknown variants), analogs shelf, dismissal memory, copy pass against the philosophy section (the review question for every string: *"would this make someone feel bad about a plant they love?"*) | A, B |
| **D** ✅ 2026-07-06 | **Plant list + handoffs.** `PlantList` persistence + list view + contribution rollup; palette-seam adapter exporting `fitForZone` in the [bed-planner](sunshower_bed_planner_spec.md)'s expected shape; entry-point audit | A–C |
...
## Build record (units A–D, 2026-07-06, `feat/plant-selector`)

Built agent-per-unit (A → B → C ∥ D) per the rounds plan. What shipped, and what the build taught us:

- **Unit A.** `ops/build-plant-data.mjs` extended; `plants.json` **159 → 309 records** (150 with `native:` blocks). **Pipeline discovery:** the cleanup-plan removal fields are a *post-build overlay* applied by four Python scripts (`apply_removal_methods.py` → `backfill_removal_sources.py` → `apply_layer_b_fields.py` → `apply_layer_c_notes.py`) — a bare build-script run wipes them. The chain is documented in the build-script header; regeneration verified byte-identical on all removal fields. Pure modules in `src/app/sunshower/plant-selector/`: `fit.ts` (tier↔range matrix on observed tokens `Full Sun`/`Partial Shade`/`Deep Shade`), `archetype.ts`, `similar.ts`, `contribution.ts`, `facets.ts`, `corpus.ts`. Corpus facts: **42 natives have empty `sun_range`** (fit read says so honestly, other signals still speak); role buckets matched the spec's counts exactly; poison-oak is `nativity: native` with **no** `native:` block → treated as unknown-tier; `soil_ph` carries the lone-`.` gremlin (passed through as string, harmless).
- **Unit B.** Route + drawer + lens + batches + search as specced; static prerender preserved (`?plant=` via Suspense-wrapped `useSearchParams`). Reroll = deterministic windows of 6, no shuffle. **Finding: natives carry zero display photos** (the iNat pull covered the invasive set only) — browse cards render an initial-tile fallback; photo top-up queued in [the backlog → Photo sourcing](sunshower_backlog.md#photo-sourcing-display-tier).
- **Unit C.** `sunshower.selectorPrefs.v1` dismissal memory (never counts dismissals — boolean per plant only). Ladder rungs all live: 🟢 full read; 🟡 proven via test fixture (activates untouched when unit E pages land); 🔴 cost read from Cal-IPC fields + warm cleanup-plan handoff + role-stand-in redirect shelf; unknown honest ("If it's thriving and you love it, that counts for a lot."). Copy pass done per the philosophy review question.
- **Unit D.** `sunshower.plantList.v1` per the data contract verbatim; list view grouped zone → status with the contribution rollup (summed host counts hedged as **"up to N"** — counts overlap across plants; real dedup waits for named-species enrichment). `paletteSeam.ts` exports `paletteForSection(labels, profile) → ranked PaletteEntry[]` + `fitForZone` for the bed planner. Entry points wired: site-inventory summary bridge, cleanup-plan §3 handoff, `/sunshower` landing links.
- **Open question 4 (batch composition):** shipped role-bucketed per the spec's default; community-/goal-bucketed remains a cheap swap — Luc's eye still wanted.
- **Tests:** 95 vitest (all against the real corpus). Checks green: lint, typecheck, test, build (all routes static).
