...
> **Status: BUILT 2026-07-11 (units S1–S3, branch `feat/bed-planner-satellite-underlay`) — ⚠️ sign-offs pending Luc's confirmation at PR review (bottom).** Written for future agents, same spec → build-record pattern as [sunshower_bed_planner_spec.md](sunshower_bed_planner_spec.md). This is a follow-on to the shipped bed planner (units A–I, PR #32): it reworks **only the base-map address on-ramp** (unit B's path 1). Nothing else in the planner changes.
...
## Build record (units S1–S3, 2026-07-10/11)

**Unit S1** (`satelliteFetch.ts`): extent math using the 512-px-tile (Mapbox GL) convention — `mPerPx = EQUATOR_M_PER_PX × cos(lat) / 2^(zoom+1)` — with `logicalPx` as the ground-extent input so the `@2x` retina suffix doubles the raster without inflating the plan rectangle. Pure `mapboxUrl` / `naipUrl` URL builders with the matching framing preset (zoom 20 ≈ 250 ft, zoom 19 ≈ 500 ft at 37.3°N). Provider ladder: Mapbox token present → Mapbox; any failure or no token → keyless USGS NAIP; both fail → friendly message routing to upload/rectangle. 20 vitest tests covering extent math, URL structure, and the provider ladder. A content-type guard was added during review: ArcGIS can return HTTP 200 with a JSON/HTML error body even when `f=image`, and storing that as the underlay would render a broken image; the guard rejects any non-`image/*` response before it reaches the blob store.

**Unit S2** (`addressImport.ts` + `BaseMapStep.tsx` reshape): `importAddress` reshaped from geocode → Overpass → feet-space projection to geocode → `fetchSatelliteImage`, with the lat/lng discarded at the module boundary — the public result carries pixels, extent, attribution, and provider only. The full Overpass block, `fetchBuildingRings`, `ringArea`, `latLngRingToFeet`, and the three Overpass constants were deleted per §Deletion list. `AddressImportSuccess` now carries `{ imageKey, widthFt, heightFt, attribution, provider }`. The keep-photo toggle (scope decision 4) routes the image to either the persistent `blobStore` (kept: plan gets `imageKey` + `imageAttribution`) or a session-only `sessionStore` (opted out: plan gets geometry only, underlay renders from `SessionUnderlay` state for this session). A keep-off fetch also clears any previously-kept `imageKey`/`imageAttribution` on the plan — a previously kept photo would otherwise outrank the session underlay; caught in review and fixed. Privacy copy shipped (`AddressPanel` blurb + footer note). Test updates: `AddressImportSuccess` fields verified; an exact-key-set privacy test confirms the success result exposes no coordinates or URLs.

**Unit S3** (`AttributionChip.tsx` new; `BaseMapStep.tsx`, `CheckStep.tsx` updated; three docs): attribution chip on all underlay surfaces — `BaseMapStep`'s editing canvas (`baseMap.imageAttribution ?? sessionUnderlay?.attribution`, covering both the kept-photo and session-only cases; the session-only case is the one where Mapbox's terms most require it since we set `attribution=false` on the URL), and `CheckStep`'s preview canvas (`baseMap.imageAttribution` only, since the preview has no session underlay). `MobileReadView` draws no canvas and needed no chip. NAIP expectation-setter copy: `result.provider === 'naip'` branches the success message to the blurry-photo note. Invariant 9 reworded per spec §Privacy sign-off 2; bed-planner spec unit B build record note added; satellite-underlay spec status updated. No deviations from the spec.
